...
public class ColorPicker : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    public void SetPointerPosition(Vector2 localPosition)
    {
        if (WrongShader()) return;

        float r = localPosition.magnitude;

        if (r < 0.5f && r > image.material.GetFloat(_HueCircleInner))
        {
            h = (Atan2(localPosition.y, localPosition.x) * recip2Pi + 1) % 1;
            pointerDownLocation = PointerDownLocation.HueCircle;
        }
        else
        {
            var size = image.material.GetFloat(_SVSquareSize);
            if (localPosition.x >= -size && localPosition.x <= size && localPosition.y >= -size && localPosition.y <= size)
            {
                s = InverseLerp(-size, size, localPosition.x);
                v = InverseLerp(-size, size, localPosition.y);
